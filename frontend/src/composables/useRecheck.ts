...
import {
  ...
  type RecheckStatusResponse,
  type RecheckExecution,
  type RecheckStats,
  type RecheckSettings,
} from '~/utils/api';
...
export function useRecheck(options: RecheckOptions) {
  const {
    owner,
    repo,
    checks = ['branch', 'clone', 'gitleaks', 'issue'],
    autoRefresh = true,
  ...
  const isIdle = computed(
    () => !status.value || status.value?.status === 'idle'
  );
  ...
  async function updateSettings(
    newSettings: Partial<RecheckSettings>
  ): Promise<RecheckSettings | null> {
    const result = await callApi(
