...
export function useTabState() {
  ...
  const activeTab = ref<TabValue>(getInitialTabState());
  ...
  const setActiveTab = (tab: TabValue | string) => {
    if (tab === 'severity' || tab === 'checkType') {
      activeTab.value = tab;
    } else {
      console.warn(
        `Invalid tab value: ${tab}. Valid values are: severity, checkType`
      );
    }
  };
