    ...
    client.interceptors.response.use(
      ...
      async error => {
        ...
        if (error.response?.status === 401) {
          logError(
            createAppError.auth(errorMessages.AUTH.UNAUTHORIZED),
            'Response Interceptor'
          );
          // Could trigger logout here
        } else if (error.response?.status === 500) {
          logError(
            createAppError.server(
          ...
          logError(
            createAppError.notFound('Resource not found'),
            'Response Interceptor'
          );
        }
...
export class ApiService {
  ...
  private validateLimit(limit: number): number {
    if (limit < 1 || limit > 1000) {
      throw createAppError.validation(errorMessages.VALIDATION.LIMIT_RANGE, {
        limit,
      });
    }
    return limit;
  ...
  private validateSort(sort: string): string {
    ...
    if (!validSorts.includes(sort)) {
      throw createAppError.validation(
        errorMessages.VALIDATION.INVALID_SORT(validSorts),
        { sort, validSorts }
      );
    }
    return sort;
  ...
  async getRepos(
    ...
    try {
      ...
      throw createAppError.api(
        errorMessages.API.FETCH_FAILED('repositories'),
        context
      );
    }
  }
  ...
  async getAlertSummary(
    ...
    try {
      if (!Array.isArray(repos) || repos.length === 0) {
        throw createAppError.validation(errorMessages.VALIDATION.EMPTY_ARRAY, {
          repos,
        });
      }

      const response = await this.client.post<AlertSummary>(
        '/api/alerts/summary',
        repos
      );
      ...
      const context: ErrorContext = {
        operation: 'fetch alert summary',
        reposCount: repos.length,
      };
      throw createAppError.api(
        errorMessages.API.FETCH_FAILED('alert summary'),
        context
      );
    }
  }
  ...
  // Alert methods
  async getAlertSummaryByCheckType(
    repos: Array<{ owner: string; repo: string }>
  ): Promise<AlertSummary> {
    try {
      if (!Array.isArray(repos) || repos.length === 0) {
      ...
      const response = await this.client.post<AlertSummary>(
        '/api/alerts/summary-by-checktype',
        repos
      );
      return response.data;
    } catch (error) {
      ...
      const context: ErrorContext = {
        operation: 'fetch alert summary by check type',
        reposCount: repos.length,
      };
      throw createAppError.api(
        errorMessages.API.FETCH_FAILED('alert summary by check type'),
        context
      );
    }
  }
  ...
  async executeGlobalRecheck(checks?: string[]): Promise<RecheckResponse> {
    try {
      const response = await this.client.post<RecheckResponse>(
        '/api/recheck/global',
        {
      ...
      const context: ErrorContext = {
        operation: 'execute global recheck',
        checks,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('Global ReCheck execution'),
        context
      );
    }
  }

  async executeRecheck(
    owner: string,
    repo: string,
    checks?: string[]
    ...
    try {
      if (!owner || !repo) {
        throw createAppError.validation('Owner and repo are required', {
          owner,
          repo,
        });
      }
      ...
      const context: ErrorContext = {
        operation: 'execute recheck',
        owner,
        repo,
        checks,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck execution'),
        context
      );
    }
  }

  async getRecheckStatus(
    owner: string,
    repo: string
  ): Promise<RecheckStatusResponse> {
    try {
      if (!owner || !repo) {
        throw createAppError.validation('Owner and repo are required', {
          owner,
      ...
      const response = await this.client.get<RecheckStatusResponse>(
        `/api/recheck/${owner}/${repo}/status`
      );
      return response.data;
    } catch (error) {
      logError(error, 'getRecheckStatus');
      const context: ErrorContext = {
        operation: 'get recheck status',
        owner,
        repo,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck status'),
        context
      );
    }
  }

  async getRecheckHistory(
    owner: string,
    repo: string,
    limit: number = 10,
    offset: number = 0
    ...
    try {
      ...
      const response = await this.client.get(
        `/api/recheck/${owner}/${repo}/history`,
        {
          params: { limit, offset },
        }
      );
      ...
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck history'),
        context
      );
    }
  }

  async getRecheckStats(
    owner: string,
    repo: string
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      const response = await this.client.get(
        `/api/recheck/${owner}/${repo}/stats`
      );
      return response.data;
    } catch (error) {
      logError(error, 'getRecheckStats');
      const context: ErrorContext = {
        operation: 'get recheck stats',
        owner,
        repo,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck stats'),
        context
      );
    }
  }

  async getRecheckSettings(
    owner: string,
    repo: string
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      const response = await this.client.get(
        `/api/recheck/${owner}/${repo}/settings`
      );
      return response.data;
    } catch (error) {
      logError(error, 'getRecheckSettings');
      const context: ErrorContext = {
        operation: 'get recheck settings',
        owner,
        repo,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck settings'),
        context
      );
    }
  }

  async updateRecheckSettings(
    owner: string,
    repo: string,
    settings: Partial<RecheckSettings>
  ): Promise<{
    success: boolean;
    message: string;
    data: RecheckSettings;
    ...
    try {
      ...
      const context: ErrorContext = {
        operation: 'update recheck settings',
        owner,
        repo,
        settings,
      };
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('ReCheck settings update'),
        context
      );
    }
  }
  ...
  async fetchData<T = unknown>(
    ...
    try {
      ...
      throw createAppError.api(
        errorMessages.API.REQUEST_FAILED('API'),
        context
      );
    }
  }
