 ...
 */
export function validateGitHubUrlParams(
  params: Partial<GitHubUrlParams>
): boolean {
  ...
  if (filePath && (!lineNumber || lineNumber < 1)) {
    console.warn(
      'GitHub URL validation failed: lineNumber must be provided and >= 1 when filePath is provided'
    );
    return false;
  }
...
export function getIssueUrl(
  ...
  if (
    !validateGitHubUrlParams({ owner, repo }) ||
    !issueNumber ||
    issueNumber < 1
  ) {
    return '#';
 ...
 */
export function processIssueNumbers(
  text: string,
  owner: string,
  repo: string
): string {
